...
public class PaymentCalculationServiceTests
{
    ...
    [Test]
    public async Task GetComplianceSchemeRegistrationFeesBySubmissionId_Ok_ReturnsResponse_AndSubstitutesSubmissionId()
    {
        var submissionId = Guid.NewGuid();
        var response = new HttpResponseMessage(HttpStatusCode.OK)
        {
            Content = _complianceSchemeCalculationResponse.ToJsonContent(_jsonOptions),
        };
        _paymentServiceApiClientMock.Setup(x => x.SendGetRequest(It.IsAny<string>())).ReturnsAsync(response);

        var result = await _systemUnderTest.GetComplianceSchemeRegistrationFeesBySubmissionId(submissionId);

        result.Should().BeEquivalentTo(_complianceSchemeCalculationResponse);
        _paymentServiceApiClientMock.Verify(
            x => x.SendGetRequest($"compliance-scheme/registration-fee/{submissionId}"),
            Times.Once);
    }

    [Test]
    public async Task GetComplianceSchemeRegistrationFeesBySubmissionId_NotFound_ReturnsNull()
    {
        _paymentServiceApiClientMock
            .Setup(x => x.SendGetRequest(It.IsAny<string>()))
            .ReturnsAsync(new HttpResponseMessage(HttpStatusCode.NotFound));
    ...
    [Test]
    public async Task GetComplianceSchemeRegistrationFeesBySubmissionId_ClientThrows_ReturnsNull()
    {
        _paymentServiceApiClientMock
            .Setup(x => x.SendGetRequest(It.IsAny<string>()))
            .ThrowsAsync(new Exception("boom"));
    ...
    [Test]
    public async Task GetComplianceSchemeRegistrationFeesBySubmissionId_ServerError_ReturnsNull()
    {
        _paymentServiceApiClientMock
            .Setup(x => x.SendGetRequest(It.IsAny<string>()))
            .ReturnsAsync(new HttpResponseMessage(HttpStatusCode.InternalServerError));
